...
describe("classifyError", () => {
	...
	it("classifies not registered as permanent", () => {
		expect(classifyError(new Error("Number not registered on WhatsApp"))).toBe(
			"permanent",
		);
	});
...
describe("withRetry", () => {
	...
	it("fails immediately on permanent error without retrying", async () => {
		...
		await expect(
			withRetry(op, "test-op", mockLogger, shortConfig),
		).rejects.toThrow("invalid jid");
	...
	it("respects server retry-after hint in log", async () => {
		...
		expect(mockLogger.warn).toHaveBeenCalledWith(
			expect.stringContaining("using server hint"),
		);
	});
